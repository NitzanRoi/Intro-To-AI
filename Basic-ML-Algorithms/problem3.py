# Classification algorithms: SVM, Logistic Regression, KNN, Decision Trees, Random Forest
...
class TrainTestModel:
    ...
    def plot_decision_boundaries_example(self, clf):
        ...
        plt.scatter(X[:, 0], X[:, 1], s=plot_symbol_size,
    ...
    def train_svm_linear(self):
        # svm (linear kernel) with grid-search for tuning the hyper-parameters:
        print('SVM Linear')
    ...
    def train_logistic_regression(self):
        # logistic regression with grid-search for tuning the hyper-parameters:
...
class DrawGraph:
    # class for drawing the given raw data
    ...
    def draw_dots(self):
        ...
        plt.scatter(pos_vector[:, 0], pos_vector[:, 1],
                    color='blue', label='one')
        plt.scatter(zero_vector[:, 0], zero_vector[:, 1],
                    color='green', label='zero')
